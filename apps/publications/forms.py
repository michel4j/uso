...
class PubWizardForm1(ModalForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.body.title = "Report a Publication"
        self.body.append(
            Div(
                Div(Field('reference', css_class="selectize"), css_class="col-sm-12 modal-update"),
                Div('code', css_class="col-sm-12"), css_class="row"
            ),
            'details',
        )
        self.footer.layout = Layout(
            StrictButton('Continue', type="submit", value="Submit", css_class='btn-primary')
        )
...
class PubWizardForm2(ModalForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.autobuttons = Layout(
            StrictButton('No, Start Over', type="submit", name="wizard_goto_step", value="0", css_class='btn-danger'),
            StrictButton('Yes, Continue', type="submit", value="Submit", css_class='btn-primary'),
        )
        self.buttons = Layout(
...
class PubWizardForm3(ModalForm):
    ...
    notes = forms.CharField(max_length=512, required=False, widget=forms.Textarea,
                            label=_("Are there any corrections to the citation details, "
                                    "or extra information about this publication that is of interest?"))
