...
def sample_state_display(sample):
    ...
    if full_state == sample.STATES.approved:
        return f"<i class='bi-check-circle-fill icon-1x icon-fw text-success'></i> {full_display}"
    elif full_state == sample.STATES.pending:
        return f"<i class='bi-hourglass icon-1x icon-fw text-muted'></i> {full_display}"
    elif full_state == sample.STATES.rejected:
        return f"<i class='bi-ban  icon-fw icon-1x text-danger'></i> {full_display}"
